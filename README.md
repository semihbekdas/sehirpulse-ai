...
# Sprint 2 — Eren'in branch çıktıları ve teslim tamamlama

**Tarih:** 6–19 Temmuz 2026

**Sprint hedefi:** Çalışan MVP'yi gerçek ekranlarla belgelemek, frontend çalıştırma yapılandırmasını tamamlamak, README ve sprint kanıtlarını teslim seviyesine getirmek ve final sprintini ölçülebilir bir backlog ile hazırlamak.

### Branch temeli

Bu çalışma arkadaşımız Eren'in `sprint-2` branch'i üzerine kuruludur. Git geçmişindeki doğrulanmış zincir şöyledir:

```text
0f3f3a9  origin/sprint-2  Eren: README taslağı, dokümanlar,
                            gerçek ekran görüntüleri ve Vite yapılandırması
    ↓
bbbf9ec  origin/main      Eren: Admin paneli görsel düzeltmesi
    ↓
c4adc3e  agent/...        Sprint 2 README tamamlama, doğrulama ve Sprint 3 planı
```

`origin/sprint-2`, mevcut agent branch'inin doğrudan atasıdır. Dolayısıyla arkadaşımızın çalışması yeniden yazılmadı veya atlanmadı; onun branch çıktıları korunarak geliştirildi.

### Puan mantığı ve gerçekleşen işler

| İş | SP | Durum | Kanıt |
| --- | ---: | --- | --- |
| Sprint 2 README taslağı, Vite yapılandırması ve bağımlılık temeli — Eren | 8 | Tamamlandı | `0f3f3a9`, `frontend/vite.config.js` |
| Gerçek uygulama ekranları ve admin görsel düzeltmesi — Eren | 5 | Tamamlandı | `docs/screenshots/`, `bbbf9ec` |
| README ve Sprint 2 kanıt dokümanlarını tamamlama | 8 | Tamamlandı | `README.md`, `docs/sprint-2.md` |
| AI/backend/frontend doğrulaması ve güvenli bağımlılık güncellemesi | 5 | Tamamlandı | `docs/test-report.md`, `frontend/package.json` |
...
Eren'in Sprint 2 branch'i ürün anlatımını, gerçek ekran görüntülerini ve frontend çalıştırma temelini sağladı. Bu temel üzerinde README tamamlandı, ürün davranışı yeniden test edildi, frontend bağımlılıkları güvenli güncel sürümlere taşındı ve Sprint 3 backlog'u çıkarıldı. Mevcut çalışan parçalar ile henüz yapılmamış özellikler ayrıştırılarak final sprintinde admin iş akışı, AI öncelik kalitesi, otomatik test ve deployment işleri P0 olarak belirlendi.
